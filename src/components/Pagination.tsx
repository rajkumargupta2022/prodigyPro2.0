...
const  Paginations: React.FC<paginationProps>=({totalRecords,page,setPage,limit,setLimit}) =>{
  ...
  return (
    <>
      ...
      <div className="d-flex flex-wrap justify-content-between align-items-center mb-3 mx-4">
          ...
          <nav>
            <ul className="pagination pagination-sm mb-0">
              <li className={`page-item ${page === 1 ? "disabled" : ""}`}>
                <button className="page-link" onClick={() => handlePage(page - 1)}>
                  Prev
                </button>
              </li>
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((num) => (
                <li key={num} className={`page-item ${page === num ? "active" : ""}`}>
                  <button className="page-link" onClick={() => handlePage(num)}>
                    {num}
                  </button>
                </li>
              ))}
