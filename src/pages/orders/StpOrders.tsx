...
function StpOrders() {
  ...
  const [stpList, setStpList] = useState<switchOrderKeys[]>([]);
  ...
  const fetchOrderData = async () => {
    ...
    if (adminUser?.ucc) {
      ...
      try {
        const res = await postRequest<switchOrderRes>(endPoints.getStpOrders, reqBody);
        if (res.success) {
          setStpList(res.data);
  ...
  return (
    <>
      ...
      {stpList?.length > 0 ? (
        ...
        <div className="m-0"><PortfolioEmpty title={"No Recent STP Orders"} body={"Your order history will appear here once you start investing. Begin your journey today!"} btnName={"Explore Funds"} btnUrl={"/all-mutual-funds"} /></div>
      )}
      {totalRecords > 9 ?
